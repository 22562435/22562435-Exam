...
find_top_names <- function(namestogo, rankednames, charts) {
    ...
    for (name in namestogo) { #!!I understand for loops are not appropriate but this is the only way i could get this to work
        ...
        if (is.na(max_growth_year)) { # If no max growth year found, skip to next name, will not happen for names provided in README
            next
        }

        # Find instances of the name in charts for the year prior because
        charts_match <- charts %>%
